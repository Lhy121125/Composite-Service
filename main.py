...
async def get_dashboard_data(user_id):
    async with aiohttp.ClientSession() as session:
        ...
        user_task = asyncio.create_task(
            fetch_data(session, user_url, "Micro-service 1")
        )
        cover_letter_task = asyncio.create_task(
            fetch_data(session, cover_letter_url, "Micro-service 2")
        )
        ...
        return {"Dashboard Data": dashboard, "Async Call Order": completion_order}
...
def get_user_details(user_id):
    user_url = (
        f"http://ec2-13-59-146-34.us-east-2.compute.amazonaws.com:8012/users/{user_id}"
    )
    cover_letter_url = (
        f"https://modular-granite-402517.uc.r.appspot.com/{user_id}/get_template_count"
    )
